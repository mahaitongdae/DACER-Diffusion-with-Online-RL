...
from relax.algorithm.diffusion_v2 import Diffv2, Diffv2OptStates, Diffv2TrainState
    ...
    def __init__(
        ...
        *,
        ...
        def stateless_update(
            key: jax.Array, state: Diffv2TrainState, data: LogpExperience
        ) -> Tuple[Diffv2OptStates, Metric]:
            obs, action, reward, next_obs, done, logp = data.obs, data.action, data.reward, data.next_obs, data.done, data.logp
            ...
            def policy_loss_fn(policy_params) -> jax.Array:
                q_mean = get_min_taret_q(obs, action)
                q_diff = q_mean - logp # logp is the prev q
                scaled_q = q_diff.clip(-3., 3.) / jnp.exp(log_alpha)
                ...
                return loss, (q_weights, next_action, q_diff)

            (total_loss, (q_weights, sampled_actions, q_diff)), policy_grads = jax.value_and_grad(policy_loss_fn, has_aux=True)(policy_params)
            ...
            info = {
                ...
                "q_diff_mean": jnp.mean(q_diff),
                "q_diff_std": jnp.std(q_diff),
